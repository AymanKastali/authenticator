...
def get_current_user(token: str = Depends(oauth2_scheme)) -> TokenUserOutDto:
    try:
    ...
    user = jwt_auth_container.token_user_controller.execute(
...
def get_current_active_user(
    current_user: TokenUserOutDto = Depends(get_current_user),
):
